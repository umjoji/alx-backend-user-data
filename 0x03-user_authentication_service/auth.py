...
class Auth:
    ...
    def register_user(self, email: str, password: str) -> User:
        """
        Register a new user and return a new user object
        Args:
            email(str): new user's email address
            password(str); new user's password
        Return:
            if no user with given email exists, retirn new user
            els raise ValueError
        """
        try:
    ...
    def get_user_from_session_id(self, session_id: str) -> Union[None, U]:
        """
        Takes a session_id and returns the corresponding user if exists
        else None
        Args:
    ...
    def get_reset_password_token(self, email: str) -> str:
        """
        Generates a reset token uuid for a user identified by the given email
        Args:
            email(str): user's email address
        Return:
    ...
    def update_password(self, reset_token: str, password: str) -> None:
        ...
        hashed = _has_password(password)
        self.db.update_user(user.id, hashed_password=hashed, reset_token=None)
